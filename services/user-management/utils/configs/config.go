...
func LoadConfig(path string, logger *logger.Logger) (*Config, error) {
	...
	// Validate essential GRPC configuration values
...
func setDefaults(v *viper.Viper) {
	...
	v.SetDefault("postgres.database", "user-db")
...
func validateGRPCConfig(grpcConfig GRPCConfig) error {
	...
	if grpcConfig.UserPort == 0 {
		return fmt.Errorf("user port is required")
	}
	if grpcConfig.AuthHost == "" {
		return fmt.Errorf("auth host is required")
	}
	if grpcConfig.AuthPort == 0 {
		return fmt.Errorf("auth port is required")
	}
	return nil
}
